...
class PyGameJoystick(object):
    def __init__(self,
        ...
        self.button_states = [0 for i in range(
...
class RCReceiver:
    ...
    def __init__(self, cfg, debug=False):
        ...
        # standard variables
        self.channels = [Channel(cfg.STEERING_RC_GPIO),
    ...
    def run(self, mode=None, recording=None):
        ...
        for channel in self.channels:
            ...
            i += 1
        if self.debug:
            print('RC CH1 signal:', round(self.signals[0], 3), 'RC CH2 signal:',
                  round(self.signals[1], 3), 'RC CH3 signal:',
        ...
        if ((self.signals[1] - self.jitter) > 0) and self.RECORD:
            # is throttle above jitter level? If so, turn on auto-record
            is_action = True
        else:
...
class JoystickCreator(Joystick):
    ...
    def poll(self):
        button, button_state, axis, axis_val = super(JoystickCreator,
                                                     self).poll()
...
class JoystickController(Part):
    ...
    def on_throttle_changes(self):
        ...
        if self.auto_record_on_throttle:
            self.recording = (
                abs(self.throttle) > self.dead_zone and self.mode == 'user')
...
class JoyStickPub(object):
    """
    Use Zero Message Queue (zmq) to publish the control messages from a local
    joystick
    """
...
class JoyStickSub(object):
    """
    Use Zero Message Queue (zmq) to subscribe to control messages from a
    remote joystick
    """
