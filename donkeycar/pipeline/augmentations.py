...
class ImageAugmentation:
    ...
    def create(cls, aug_type: str, config: Config) -> iaa.meta.Augmenter:
        ...
        if aug_type == 'CROP':
            ...
            b_limit = getattr(config, 'BRIGHTNESS_RANGE', 0.2)
            logger.info(f'Creating augmentation {aug_type} {b_limit}')
            return RandomBrightnessContrast(brightness_limit=b_limit)

        elif aug_type == 'BLUR':
            b_limit = getattr(config, 'AUG_BLUR_RANGE', (3, 7))
            logger.info(f'Creating augmentation {aug_type} {b_limit}')
            return GaussianBlur(blur_limit=b_limit)

    # Parts interface
    def run(self, img_arr):
        aug_img_arr = self.augmentations(img_arr)["image"]
        return aug_img_arr
