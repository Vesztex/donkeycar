...
class TestTubWriter(unittest.TestCase):
    ...
    def test_make_paths_absolute(self):
        ...
        assert abs_record_dict['file_path'] \
            == os.path.join(self.path, rel_file_name)

    def test_tub_meta(self):
        meta = ["location:Here", "task:sometask"]
        tub = Tub(self.path, inputs=['file_path'],
                  types=['image'], user_meta=meta)
        t2 = Tub(self.path)
    ...
    def test_tub_like_driver(self):
        """ The manage.py/donkey2.py drive command creates a tub using
            TubHandler, so test that way.
        """
        os.makedirs(self.tempfolder)
        meta = ["location:Here2", "task:sometask2"]
        th = TubHandler(self.tempfolder)
        tub = th.new_tub_writer(inputs=self.inputs, types=self.types,
                                user_meta=meta)
        t2 = Tub(tub.path)
